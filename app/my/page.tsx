...
type MyIcon = "history"|"pb"|"shoe"|"shoeGuide"|"treadmill"|"saved"|"race"|"raceSearch"|"crew"|"profile"|"account";
function HubCard({href,icon,label,title,description}:{href:string;icon:MyIcon;label:string;title:string;description:string}){
...
export default async function MyPage() {
  ...
  return <main className="wrap hubPage myHubPage">
    ...
    <section className="pageHubGrid">
      <HubCard href="/my/history" icon="history" label="HISTORY" title="러닝 기록" description="최근 러닝과 상세 기록" />
      <HubCard href="/my/pb" icon="pb" label="PERSONAL BEST" title="내 PB" description="1K·3K·5K·10K·트랙" />
      <HubCard href="/my/shoes" icon="shoe" label="SHOE MILEAGE" title="내 러닝화" description="신발별 누적 거리·교체 시점" />
      <HubCard href="/shoes" icon="shoeGuide" label="SHOE GUIDE" title="러닝화 추천" description="용도별 랭킹·드롭·판매 일정" />
      <HubCard href="/run/treadmill" icon="treadmill" label="TREADMILL" title="트레드밀 계산기" description="목표 페이스를 속도로 변환" />
      <HubCard href="/favorites" icon="saved" label="SAVED" title="저장함" description={`찜한 코스 ${favCount || 0}개 · 장소 · 일정`} />
      <HubCard href="/races/my" icon="race" label="MY RACE" title="내 대회 일정" description={`저장한 대회 ${raceCount || 0}개 · 참가 상태`} />
      <HubCard href="/races" icon="raceSearch" label="RACE FINDER" title="대회 찾기" description="전국 일정·Google Form 접수" />
      <HubCard href="/dashboard" icon="crew" label="CREW" title="크루" description="활동·출석·대회 관리" />
      <HubCard href="/my/profile" icon="profile" label="PROFILE" title="프로필" description="표시 이름 관리" />
      <HubCard href="/my/account" icon="account" label="ACCOUNT" title="계정 관리" description="개인정보·계정 삭제 요청" />
    </section>
