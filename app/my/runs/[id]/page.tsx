...
export default async function RunHistoryPage({params}:{params:Promise<{id:string}>}){
 ...
 return <main className="wrap detailPage ttwittunDetailPage runDetailPage"><header className="top detailTop"><Brand/><div className="nav"><Link className="btn ghost" href="/my">← MY</Link>{!isFree&&run.course_id&&<Link className="btn" href={`/courses/${run.course_id}`}>코스 상세</Link>}<Link className="btn" href="/run/free">🏃 새 러닝</Link></div></header>
 ...
 <div className="mobilePrimaryBar"><Link className="btn ghost" href="/my">← MY</Link><Link className="btn" href="/run/free">새 러닝 시작</Link></div>
 </main>
}
