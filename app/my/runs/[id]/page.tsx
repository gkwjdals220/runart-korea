...
export default async function RunHistoryPage({params}:{params:Promise<{id:string}>}){
 ...
 return <main className="wrap detailPage ttwittunDetailPage runDetailPage"><header className="top detailTop"><Brand/><div className="nav"><Link className="btn ghost" href="/my"><TtwittunButtonIcon name="back" compact/>MY</Link>{!isFree&&run.course_id&&<Link className="btn" href={`/courses/${run.course_id}`}><TtwittunButtonIcon name="map" compact/>코스 상세</Link>}<Link className="btn" href="/run/free"><TtwittunButtonIcon name="run" compact/>새 러닝</Link></div></header>
 ...
 <div className="mobilePrimaryBar"><Link className="btn ghost" href="/my"><TtwittunButtonIcon name="back" compact/>MY</Link><Link className="btn" href="/run/free"><TtwittunButtonIcon name="run" compact/>새 러닝</Link></div>
 </main>
}
