...
export default async function PbPage(){
 ...
 return <main className="wrap mobileSubPage pbPage"><section className="compactPageHero"><span className="eyebrow">PERSONAL BEST</span><h1>내 PB</h1><p className="muted">거리별 최고 기록만 빠르게 확인합니다.</p></section><div className="pbPageGrid">{cards.map(([label,value])=><div className="card pbPageCard" key={label}><small>{label}</small><b>{value?fmt(value):"--:--"}</b><span>{value?"개인 최고":"기록 없음"}</span></div>)}</div><div className="pageBottomActions pbTextOnlyActions"><Link className="btn pbTextOnlyAction" href="/run/free">기록 시작</Link><Link className="btn ghost pbTextOnlyAction" href="/run/track">트랙런</Link></div><style>{`.pbTextOnlyActions .pbTextOnlyAction{background-image:none!important}.pbTextOnlyActions .pbTextOnlyAction::before,.pbTextOnlyActions .pbTextOnlyAction::after{content:none!important;display:none!important;background:none!important;background-image:none!important}`}</style></main>
}
