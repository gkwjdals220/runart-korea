...
export default async function Home(){
 ...
 <header className="top simpleTop"><Brand/><nav className="homeDesktopNav" aria-label="홈 주요 메뉴"><a href="#recommend">추천</a><a href="#waterfront">수변</a><a href="#art-runs">GPS 아트</a><a href="#run-eat">RUN + EAT</a><a href="#explore">코스 탐색</a><Link href="/run/free">자유 러닝</Link><Link href="/dashboard">크루</Link></nav><div className="nav"><Link className="btn ghost" href="/favorites">♡ 찜</Link><Link className="btn" href={user?"/my":"/login"}>{user?"MY":"로그인"}</Link></div></header>
 <section className="simpleIntro"><span className="introPill">TTWITTUN · RUN · RECORD</span><h1>오늘 달릴 코스를 고르거나,<br className="mobileBreak"/> 바로 기록을 시작하세요.</h1><p>코스 탐색부터 주차·화장실·라이브 러닝·개인 기록·맛집까지 하나의 흐름으로 이어집니다.</p><div className="heroActions"><Link className="btn" href="/run/free">🏃 자유 러닝 시작</Link><a className="btn ghost" href="#explore">🔎 코스 찾기</a><a className="btn ghost" href="#recommend">✨ 추천 코스</a></div></section>
 <section className="mobilePrimaryActions" aria-label="모바일 빠른 시작"><a href="#explore"><span>🔎</span><b>코스 찾기</b><small>지도·검색</small></a><Link href="/run/free" className="primary"><span>▶</span><b>RUN 시작</b><small>바로 기록</small></Link><Link href="/my"><span>⌁</span><b>내 기록</b><small>PB·히스토리</small></Link><Link href="/dashboard"><span>◉</span><b>크루</b><small>출석·대회</small></Link></section>
 <div className="homeQuickNav" aria-label="빠른 이동"><Link href="/run/free">🏃 자유 러닝</Link><a href="#recommend">✨ 추천</a><a href="#waterfront">🌊 수변</a><a href="#art-runs">🎨 GPS 아트</a><a href="#run-eat">🍴 RUN + EAT</a><a href="#explore">🔎 전체 탐색</a></div>
 <section className="serviceFlowBar" aria-label="TTWITTUN 이용 흐름"><a href="#explore"><b>1</b><span><strong>코스 선택</strong><small>검색·지도·필터</small></span></a><span className="flowArrow">→</span><a href="#explore"><b>2</b><span><strong>출발 준비</strong><small>주차·화장실</small></span></a><span className="flowArrow">→</span><Link href="/run/free"><b>3</b><span><strong>라이브 러닝</strong><small>GPS·페이스·자동랩</small></span></Link><span className="flowArrow">→</span><Link href="/my"><b>4</b><span><strong>개인 기록</strong><small>MY · 스플릿 · 히스토리</small></span></Link></section>
 <InteractiveRunBoard/>
 <section className="exploreReveal" id="explore"><div className="simpleSectionHead"><div><h2>코스 탐색</h2><p>검색하고, 지도에서 확인하고, 바로 출발하세요.</p></div><a href="#top">맨 위로 ↑</a></div><ExplorerPresetButtons/><CourseExplorer courses={normalized as any} userId={user?.id||null} favoriteIds={favoriteIds}/></section>
