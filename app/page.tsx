...
export default async function Home(){
 ...
 if(user){
  ...
  const today=new Date().toISOString().slice(0,10);const {data:races}=await sb.from("runart_public_race_participation").select("race_name,race_date,status").eq("user_id",user.id).in("status",["applied","going"]).gte("race_date",today).order("race_date",{ascending:true}).limit(1);nextRace=races?.[0]||null;
 }
 const promos=[
  ...
  nextRace?{href:"/races",eyebrow:"NEXT RACE",title:nextRace.race_name,description:`${nextRace.race_date} · 내 참가 일정으로 저장됨`,cta:"대회 확인",icon:"🏁",className:"promoRace"}:{href:"/races",eyebrow:"RACE",title:"다음 목표를 정해볼까요?",description:"다가오는 전국 러닝 대회와 접수 정보를 확인하세요.",cta:"대회 보기",icon:"🏁",className:"promoRace"},
 ...
 return <main className="wrap simpleHome actionHome">
  ...
  <section className="homeActionSection compactHomeSection"><div className="homeSectionTitle"><div><small>FOR YOU</small><h2>자주 찾는 메뉴</h2></div></div><div className="homeDirectList"><Link href="/favorites"><span>♡</span><div><b>저장함</b><small>찜한 코스 {favoriteCount}개 · 장소 · 일정</small></div><em>›</em></Link><Link href="/run/track"><span>🏟</span><div><b>트랙런</b><small>400m 자동랩 · 인터벌</small></div><em>›</em></Link><Link href="/dashboard/activity"><span>◉</span><div><b>크루 최근 활동</b><small>최근 러닝과 참여 현황</small></div><em>›</em></Link></div></section>
