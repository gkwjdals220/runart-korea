...
export default async function Manage(){
 ...
 const {data:pending}=await sb.from("runart_courses")
 ...
 const ids=Array.from(new Set((joinReqs||[]).map((x:any)=>x.user_id)));
 const {data:profiles}=ids.length?await sb.from("runart_profiles").select("user_id,display_name").in("user_id",ids):{data:[] as any[]};
 ...
 const memberIds=Array.from(new Set((members||[]).map((x:any)=>x.user_id)));
 const {data:memberProfiles}=memberIds.length?await sb.from("runart_profiles").select("user_id,display_name").in("user_id",memberIds):{data:[] as any[]};
 const memberNames=new Map((memberProfiles||[]).map((p:any)=>[p.user_id,p.display_name]));
 ...
 return <main className="wrap">
  ...
  <section className="hero compact"><h1>운영 센터</h1><p className="muted">크루원 가입 승인과 코스 제보를 관리합니다.</p></section>
  <div className="card"><b>현재 권한: {role}</b>{ownedCrew&&<span className="muted"> · {ownedCrew.name}</span>}</div>

  <section className="section"><h2>크루 가입 신청</h2>
   {(joinReqs||[]).length?(joinReqs||[]).map((x:any)=><article className="card moderationCard" key={x.id}><div><h3>{names.get(x.user_id)||"신규 러너"}</h3><p className="muted">{x.message||"가입 메시지 없음"}</p><small className="muted">{new Date(x.created_at).toLocaleString("ko-KR")}</small></div><JoinRequestActions requestId={x.id}/></article>):<div className="card muted">대기 중인 가입 신청이 없습니다.</div>}
  </section>

  <section className="section"><h2>현재 크루원</h2>
   <div className="grid">{(members||[]).map((m:any)=><div className="card" key={m.user_id}><h3>{memberNames.get(m.user_id)||"러너"}</h3><p className="muted">권한: {m.role}</p></div>)}</div>
  </section>
