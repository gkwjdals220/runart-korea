...
export default async function Manage(){
 ...
 const admin=(memberships||[]).some((m:any)=>["owner","admin"].includes(m.role));
 ...
 return <main className="wrap">
  <header className="top"><Brand/><div className="nav"><Link className="btn ghost" href="/dashboard">대시보드</Link><Link className="btn ghost" href="/">지도</Link></div></header>
  <section className="hero compact"><h1>운영 센터</h1><p className="muted">코스 제보를 검수하고 공개 여부를 결정합니다.</p></section>
  <div className="card"><b>현재 권한: {memberships?.[0]?.role||"크루 미가입"}</b></div>
