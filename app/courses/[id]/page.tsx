...
export default async function CoursePage({params}:{params:Promise<{id:string}>}){
 ...
 return <main className="wrap detailPage">
   ...
   <div className="journeyBreadcrumb" aria-label="러닝 진행 단계"><span className="done">1 코스 선택</span><span className="active">2 출발 준비</span><span>3 라이브 러닝</span><span>4 기록 저장</span></div>
   <section className="detailHero card discoveryDetail" id="overview"><div><span className="eyebrow">RUNART COURSE</span><div className="metaRow">{course.course_type==="art"&&<span className="tag">🎨 {course.art_shape}</span>}<span>{course.region} {course.city}</span>{course.verified&&<span className="tag">✓ 검증 코스</span>}</div><h1>{course.name}</h1><p className="muted">{Number(course.distance_km).toFixed(1)}km · 난이도 {"★".repeat(course.difficulty)}{course.start_name?` · ${course.start_name} 출발`:""}</p><div className="metaRow">{(course.tags||[]).slice(0,6).map((t:string)=><span className="tag" key={t}>#{t}</span>)}</div><div className="actions detailHeroActions"><a className="btn" href="#run-start">🏁 출발 준비하기</a><Link className="btn ghost" href={`/?course=${id}#explore`}>🗺️ 코스 지도</Link></div></div><div className="score"><b>{avg?avg.toFixed(1):"-"}</b><span>★ / 후기 {(reviews||[]).length}</span></div></section>
   ...
   <div className="mobilePrimaryBar"><Link className="btn ghost" href={`/?course=${id}#explore`}>🗺️ 지도</Link><a className="btn" href="#run-start">🏁 출발 준비</a></div>
 </main>
}
