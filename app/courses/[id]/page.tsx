...
export default async function CoursePage({params}:{params:Promise<{id:string}>}){
 ...
 return <main className="wrap detailPage">
   <header className="top detailTop"><Brand/><div className="nav"><Link className="btn ghost" href="/#explore">← 코스 탐색</Link><Link className="btn ghost" href="/favorites">♡ 찜</Link><Link className="btn" href="/dashboard">크루</Link></div></header>
   <nav className="detailQuickNav" aria-label="코스 상세 빠른 이동"><a href="#overview">요약</a><a href="#run-start">🏁 출발</a><a href="#ready">준비정보</a><a href="#parking">🚗 주차</a><a href="#after-run">🍚 러닝 후</a><a href="#reviews">후기</a></nav>
   <section className="detailHero card discoveryDetail" id="overview"><div><span className="eyebrow">RUNART COURSE</span><div className="metaRow">{course.course_type==="art"&&<span className="tag">🎨 {course.art_shape}</span>}<span>{course.region} {course.city}</span>{course.verified&&<span className="tag">✓ 검증 코스</span>}</div><h1>{course.name}</h1><p className="muted">{Number(course.distance_km).toFixed(1)}km · 난이도 {"★".repeat(course.difficulty)}{course.start_name?` · ${course.start_name} 출발`:""}</p><div className="metaRow">{(course.tags||[]).slice(0,6).map((t:string)=><span className="tag" key={t}>#{t}</span>)}</div><div className="actions" style={{marginTop:14}}><a className="btn" href="#run-start">🏁 지금 출발하기</a><Link className="btn ghost" href={`/?course=${id}#explore`}>🗺️ 지도에서 보기</Link></div></div><div className="score"><b>{avg?avg.toFixed(1):"-"}</b><span>★ / 후기 {(reviews||[]).length}</span></div></section>
   ...
   <div className="mobilePrimaryBar"><Link className="btn ghost" href={`/?course=${id}#explore`}>🗺️ 지도</Link><a className="btn" href="#run-start">🏁 지금 출발</a></div>
 </main>
}
