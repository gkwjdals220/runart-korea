...
export default async function CoursePage({params}:{params:Promise<{id:string}>}){
 ...
 if(user){
   ...
   const {data:pf}=await sb.from("runart_place_favorites").select("place_id").eq("user_id",user.id);
   placeFavoriteIds=(pf||[]).map((x:any)=>x.place_id);
 }
 ...
 return <main className="wrap">
   ...
   <section className="detailHero card discoveryDetail">
     <div><span className="eyebrow">RUNART COURSE</span><div className="metaRow">{course.course_type==="art"&&<span className="tag">🎨 {course.art_shape}</span>}<span>{course.region} {course.city}</span>{course.verified&&<span className="tag">✓ 검증 코스</span>}</div>
     <h1>{course.name}</h1><p className="muted">{Number(course.distance_km).toFixed(1)}km · 난이도 {"★".repeat(course.difficulty)}{course.start_name?` · ${course.start_name} 출발`:""}</p><div className="metaRow">{(course.tags||[]).slice(0,6).map((t:string)=><span className="tag" key={t}>#{t}</span>)}</div></div>
   ...
   <div className="grid2 section">
     <div className="card"><span className="eyebrow">ROUTE INFO</span><h3>코스 한눈에 보기</h3><div className="miniStats"><span><b>{course.surface||"-"}</b>노면</span><span><b>{course.elevation_gain_m??"-"}</b>상승고도(m)</span><span><b>{course.loop_type==="loop"?"순환":course.loop_type==="out_back"?"왕복":course.loop_type==="point_to_point"?"편도":"-"}</b>코스 형태</span></div><p className="muted" style={{marginTop:14}}>지도 메인 화면에서 실제 폴리라인과 함께 확인할 수 있습니다.</p><Link className="btn ghost" href={`/?course=${id}#explore`}>지도에서 보기</Link></div>
     {user?<FavoriteReview userId={user.id} courseId={id} initialFavorite={fav} initialRating={myRating} initialBody={myBody}/>:<div className="card"><h3>저장·후기</h3><p className="muted">로그인하면 코스를 찜하고 별점/후기를 남길 수 있습니다.</p><Link className="btn" href="/login">로그인</Link></div>}
   </div>
   <NearbyPlaces courseId={id} userId={user?.id||null} initialFavoriteIds={placeFavoriteIds}/>
   <section className="section"><div className="sectionHead"><div><span className="eyebrow">RUNNER REVIEW</span><h2>러너 후기</h2></div></div><div className="reviewGrid">
