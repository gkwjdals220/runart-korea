...
export default async function CoursePage({params}:{params:Promise<{id:string}>}){
 ...
 return <main className="wrap detailPage ttwittunDetailPage courseDetailPage">
   ...
   <section className="detailHero card discoveryDetail" id="overview"><div><span className="eyebrow">TTWITTUN COURSE</span><div className="metaRow">{course.course_type==="art"&&<span className="tag">🎨 {course.art_shape}</span>}<span>{course.region} {course.city}</span>{course.verified&&<span className="tag">✓ 검증 코스</span>}</div><h1>{course.name}</h1><p className="muted">{Number(course.distance_km).toFixed(1)}km · 난이도 {"★".repeat(course.difficulty)}{course.start_name?` · ${course.start_name} 출발`:""}</p><div className="metaRow">{(course.tags||[]).slice(0,6).map((t:string)=><span className="tag" key={t}>#{t}</span>)}</div><div className="actions detailHeroActions"><Link className="btn" href={`/run/${id}`}><TtwittunButtonIcon name="run"/>바로 러닝 시작</Link><a className="btn ghost" href="#run-start"><TtwittunButtonIcon name="save"/>출발 준비</a><Link className="btn ghost" href={`/explore?course=${id}`}><TtwittunButtonIcon name="map"/>코스 지도</Link><CourseShareButton courseId={id} courseName={course.name} distanceKm={Number(course.distance_km||0)} region={course.region} city={course.city}/></div></div><div className="score"><b>{avg?avg.toFixed(1):"-"}</b><span>★ / 후기 {(reviews||[]).length}</span></div></section>
   ...
   <div className="mobilePrimaryBar"><CourseShareButton courseId={id} courseName={course.name} distanceKm={Number(course.distance_km||0)} region={course.region} city={course.city}/><Link className="btn ghost" href={`/explore?course=${id}`}><TtwittunButtonIcon name="map" compact/>지도</Link><Link className="btn" href={`/run/${id}`}><TtwittunButtonIcon name="run" compact/>RUN 시작</Link></div>
 </main>
}
