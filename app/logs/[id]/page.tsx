...
export default async function EditLogPage({params}:{params:Promise<{id:string}>}){
 ...
 return <main className="wrap"><header className="top"><Brand/><div className="nav"><Link className="btn ghost" href="/dashboard">← 대시보드</Link><LogoutButton/></div></header><section className="hero compact"><h1>러닝 기록 수정</h1><p className="muted">날짜·코스·거리·메모·실제 참여자를 다시 편집할 수 있습니다.</p></section><div style={{maxWidth:760}}><EditRunLogForm logId={id} initialCourseId={log.course_id} initialDate={log.run_date} initialDistance={log.actual_distance_km?Number(log.actual_distance_km):null} initialMemo={log.memo} initialParticipants={(parts||[]).map((p:any)=>p.user_id)} courses={(courses||[]).map((c:any)=>({...c,distance_km:Number(c.distance_km)}))} members={members as any}/></div></main>
}
