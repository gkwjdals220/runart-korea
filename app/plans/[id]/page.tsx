...
export default async function RunEatPlanPage({params}:{params:Promise<{id:string}>}){
 ...
 return <main className="wrap">
  <header className="top"><Brand/><div className="nav"><Link className="btn ghost" href="/">코스 탐색</Link><Link className="btn ghost" href="/favorites">내 찜</Link></div></header>
  <section className="hero compact"><div><span className="eyebrow">RUN + EAT</span><h1>{plan.title}</h1><p className="muted">달리고, 먹고, 하루 코스를 한 번에 공유하세요.</p></div><SharePlanButton title={plan.title}/></section>
