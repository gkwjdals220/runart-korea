...
export default async function RunEatPlanPage({params}:{params:Promise<{id:string}>}){
 ...
 return <main className="wrap detailPage ttwittunDetailPage planDetailPage">
  ...
  <section className="section card planNextAction"><h3>이 조합으로 달려볼까요?</h3><p className="muted">코스를 확인한 뒤 바로 러닝을 시작하고, 완주 후 저장한 장소로 이어가세요.</p><Link className="btn" href={`/courses/${course?.id}`}>코스 출발 준비 보기</Link></section>
  <div className="mobilePrimaryBar"><Link className="btn ghost" href={`/explore?course=${course?.id}`}>지도</Link><Link className="btn" href={`/courses/${course?.id}`}>코스 출발 준비</Link></div>
 </main>
}
