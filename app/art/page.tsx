...
export default async function ArtPage(){
 ...
 return <main className="wrap"><header className="top"><Brand/><div className="nav"><Link className="btn ghost" href="/">전국 지도</Link><Link className="btn" href="/dashboard">뛰뚠뛰뚠</Link></div></header>
 <section className="hero compact"><div><span className="eyebrow">GPS ART COLLECTION</span><h1>그리기 런 도감 🎨</h1><p className="muted">붕어빵·하트·동물·문자 등 달린 궤적이 그림이 되는 펀런 코스만 모았습니다.</p></div><div className="heroStats"><b>{courses?.length||0}</b><span>작품</span><b>{completed}</b><span>크루 완주</span><b>{rate}%</b><span>수집률</span></div></section>
