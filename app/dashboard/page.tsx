...
export default async function Dashboard(){
 ...
 const {data:logsData}=await sb.from("runart_course_logs")
  .select("id,run_date,actual_distance_km,memo,photo_path,course_id,created_by,runart_courses(name,distance_km)")
 ...
 return <main className="wrap">
  <header className="top"><Brand/><div className="nav">
   ...
   <Link className="btn ghost" href="/races">대회 참여</Link>
   <Link className="btn ghost" href="/submit">코스 제보</Link>
   {canManage&&<Link className="btn" href="/manage">크루 관리</Link>}
   <LogoutButton/>
  </div></header>
  ...
  <section className="section"><div className="grid2"><div><h2>내 참여 기록</h2><div className="stats"><div className="stat"><b>{myLogs.length}</b><span className="muted">참여 러닝</span></div><div className="stat"><b>{myDone.size}</b><span className="muted">완주 코스</span></div><div className="stat"><b>{myKm.toFixed(1)}</b><span className="muted">누적 km</span></div><div className="stat"><b>{(courses||[]).filter(c=>c.course_type==="art"&&myDone.has(c.id)).length}</b><span className="muted">GPS 아트</span></div></div></div><ProfileEditor userId={user.id} initialName={myName}/></div></section>
  ...
  <section className="section"><div className="grid2">
   ...
   <div className="card"><h3>최근 크루 러닝</h3><p className="muted">총 {logs.length}회 · 참여거리 합계 {crewKm.toFixed(1)}km</p>
    {logs.length?logs.slice(0,12).map((l:any)=><div className="course" key={l.id}>
     ...
     {(canManage||l.created_by===user.id)&&<Link className="textLink" href={`/logs/${l.id}`}>기록 편집 →</Link>}
