...
export default async function RacesPage(){
 ...
 return <main className="wrap racePage liveRacePage"><header className="top compactPageTop"><Brand/><div className="nav">{hasCrew&&<Link className="btn ghost" href="/races/crew">크루 대회</Link>}<Link className="btn ghost" href={user?"/my":"/login"}>{user?"MY":"로그인"}</Link></div></header><section className="compactPageHero raceLiveHero"><span className="eyebrow">RACE CALENDAR</span><h1>지금 신청할 러닝 대회</h1><p className="muted">다가오는 국내 러닝·마라톤 대회를 찾아보고 관심·신청·참가 여부를 내 기록으로 남겨두세요.</p></section><PublicRaceBrowser userId={user?.id||null} initialSaved={(saved||[]) as any}/></main>
}
