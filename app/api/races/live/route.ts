...
function isoDate(raw:string){
 const dot=raw.match(/(20\d{2})\s*[.\/-]\s*(\d{1,2})\s*[.\/-]\s*(\d{1,2})/);
 const kor=raw.match(/(20\d{2})\s*년\s*(\d{1,2})\s*월\s*(\d{1,2})\s*일/);
 const m=dot||kor;if(!m)return null;return `${m[1]}-${m[2].padStart(2,"0")}-${m[3].padStart(2,"0")}`
}
function allDates(raw:string){
 const found:string[]=[];
 const re=/(20\d{2}\s*(?:[.\/-]\s*\d{1,2}\s*[.\/-]\s*\d{1,2}|년\s*\d{1,2}\s*월\s*\d{1,2}\s*일))/g;
 let m:RegExpExecArray|null;while((m=re.exec(raw))){const d=isoDate(m[1]);if(d)found.push(d)}return found
}
...
function registrationWindow(t:string,eventDate:string){
 const idx=t.indexOf("접수");if(idx<0)return {start:null,deadline:null};
 const dates=allDates(t.slice(idx)).filter(d=>d!==eventDate);
 if(dates.length>=2)return {start:dates[0],deadline:dates[1]};
 if(dates.length===1){
  if(/까지|마감/.test(t.slice(idx)))return {start:null,deadline:dates[0]};
  return {start:null,deadline:dates[0]};
 }
 return {start:null,deadline:null}
}
function statusOf(t:string,start:string|null,deadline:string|null){
 const now=new Date();const today=new Date(now.getFullYear(),now.getMonth(),now.getDate()).getTime();
 if(/접수\s*마감|접수완료/.test(t))return "closed";
 if(start&&new Date(`${start}T00:00:00+09:00`).getTime()>today)return "upcoming";
 if(deadline&&new Date(`${deadline}T23:59:59+09:00`).getTime()<today)return "closed";
 if(/접수/.test(t)&&deadline)return "open";
 return "unknown"
}
...
export async function GET(){
 try{
  const r=await fetch(SOURCE_URL,{headers:{"user-agent":"TTWITTUN-RaceIndex/1.1 (+https://runart-korea.vercel.app)"},next:{revalidate:1800}});
  if(!r.ok)throw new Error(`source ${r.status}`);
  const html=await r.text(),rows:any[]=[];
  const re=/<a\b([^>]*)href=["']([^"']+)["']([^>]*)>([\s\S]*?)<\/a>/gi;let m:RegExpExecArray|null;
  while((m=re.exec(html))){
   const txt=textOf(m[4]);const eventDates=allDates(txt);const date=eventDates[0]||null;if(!date)continue;
   const name=cleanName(txt,date);if(name.length<4||/^전체$/.test(name))continue;
   ...
   const window=registrationWindow(txt,date),dists=distances(txt),key=createHash("sha1").update(`${name}|${date}|${official}`).digest("hex").slice(0,20);
   rows.push({source_key:`rung:${key}`,name,race_date:date,region,distance_options:dists,registration_start:window.start,registration_deadline:window.deadline,registration_status:statusOf(txt,window.start,window.deadline),official_url:official,registration_text:(txt.includes("접수")?txt.slice(txt.indexOf("접수")):"").slice(0,180),source_name:"rung",source_url:SOURCE_URL});
  }
