...
async function nearestParking(origin:Point){
  ...
  endpoint.searchParams.set("query","주차장");
...
export async function GET(req:NextRequest){
  ...
  const type=req.nextUrl.searchParams.get("type")==="parking"?"parking":"toilet";
  if(!Number.isFinite(lat)||!Number.isFinite(lng)||lat<-90||lat>90||lng<-180||lng>180){
    return NextResponse.json({error:"valid lat/lng required"},{status:400});
  }
  const origin={lat,lng};
  try{
    const result=type==="parking"?await nearestParking(origin):await nearestToilets(origin);
    ...
    return NextResponse.json({type,origin,facilities:[],nearest:null,warning:type==="toilet"?"근처 화장실 정보를 불러오지 못했습니다.":"근처 주차장 정보를 불러오지 못했습니다."},{status:200});
  }
}
