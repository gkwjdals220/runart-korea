...
export default function NearbyPlaces({courseId,courseName,userId,initialFavoriteIds=[]}:{courseId:string;courseName:string;userId?:string|null;initialFavoriteIds?:string[]}){
 ...
 return <section className="section nearbySection" id="after-run">
  <div className="sectionHead"><div><span className="eyebrow">AFTER RUN</span><h2>뛰고 나서 어디 갈까?</h2><p className="muted">코스 중심 최대 5km에서 후기와 TTWITTUN 러너 행동을 함께 반영해 추천합니다.</p></div><div className="segmented" aria-label="장소 종류"><button type="button" aria-pressed={tab==="restaurant"} className={tab==="restaurant"?"on":""} onClick={()=>setTab("restaurant")}>🍚 맛집</button><button type="button" aria-pressed={tab==="cafe"} className={tab==="cafe"?"on":""} onClick={()=>setTab("cafe")}>☕ 카페</button></div></div>
  {!data?<div className="card muted" role="status">주변 장소를 찾는 중...</div>:<><div className="nearbyTrustNote"><b>TTWITTUN 러너 점수</b><span>직접 추천 · 찜 · RUN + EAT 저장 · 외부 후기 · 거리</span><small>{data.runner_score_note||data.review_signal_note||"러너 행동 데이터가 쌓일수록 TTWITTUN 내부 신호의 비중이 커집니다."}</small></div><div className="nearbySortBar" aria-label="장소 정렬"><button type="button" aria-pressed={sort==="runner"} className={sort==="runner"?"on":""} onClick={()=>setSort("runner")}>🏃 러너 추천</button><button type="button" aria-pressed={sort==="popular"} className={sort==="popular"?"on":""} onClick={()=>setSort("popular")}>🔥 러닝 후 인기</button><button type="button" aria-pressed={sort==="reviews"} className={sort==="reviews"?"on":""} onClick={()=>setSort("reviews")}>💬 후기 많은 순</button><button type="button" aria-pressed={sort==="distance"} className={sort==="distance"?"on":""} onClick={()=>setSort("distance")}>📍 가까운 순</button></div><div className="placeRail">{places.map((p,i)=>{const favoriteKey=p.saved_place_id||p.id;return <article className="placeCard runnerPlaceCard" key={`${p.id}-${i}`}><div className="placeCardTop"><div className="placeBadge">{p.curated?"TTWITTUN PICK":Number(p.runner_score||0)>=30?"RUNNER PICK":p.review_signal&&p.review_signal>0?"USER SIGNAL":p.verified?"검증":"주변"}</div>{p.runner_score!=null&&<div className="runnerScore"><b>{p.runner_score}</b><span>러너점수</span></div>}</div><h3>{p.name}</h3><p className="muted">{p.address||"주소 정보 없음"}</p><div className="metaRow"><span>{p.distance_m!=null?`${p.distance_m<1000?p.distance_m+"m":(p.distance_m/1000).toFixed(1)+"km"}`:"거리 미정"}</span>{p.walking_minutes&&<span>도보 {p.walking_minutes}분</span>}{p.review_signal!=null&&p.review_signal>0&&<span>후기 신호 {p.review_signal.toLocaleString()}+</span>}</div>{(Number(p.favorite_count||0)>0||Number(p.plan_count||0)>0)&&<div className="runnerSignals">{Number(p.favorite_count||0)>0&&<span>♥ 러너 찜 {p.favorite_count}</span>}{Number(p.plan_count||0)>0&&<span>🍽 RUN + EAT {p.plan_count}</span>}</div>}{p.editorial_note&&<p className="placeNote">{p.editorial_note}</p>}{p.review_samples&&p.review_samples.length>0&&<div className="reviewSignalBox"><b>실사용 후기 참고</b>{p.review_samples.slice(0,2).map((r,j)=><a key={`${r.url}-${j}`} href={r.url} target="_blank" rel="noreferrer"><span>{r.title}</span><small>{r.blogname||"블로그 후기"}</small></a>)}</div>}<div className="actions placeActionBar"><button disabled={!!busy} className="btn runEatAction" type="button" onClick={()=>savePlan(p)}>{busy===`plan:${p.id}`?"일정 저장 중…":"RUN + EAT 저장"}</button><button disabled={!!busy} type="button" aria-pressed={favorites.has(favoriteKey)} className={`heartBtn ${favorites.has(favoriteKey)?"saved":""}`} onClick={()=>toggle(p)}>{busy===p.id?"저장 중…":favorites.has(favoriteKey)?"♥ 찜함":"♡ 찜"}</button>{p.source_url&&<a className="btn ghost" href={p.source_url} target="_blank" rel="noreferrer">지도 상세</a>}</div></article>})}{!places.length&&<div className="card muted emptyState">아직 연결된 {tab==="restaurant"?"맛집":"카페"}이 없습니다.{!data.configured&&" 장소 API 키를 연결하면 코스 주변을 자동 검색할 수 있습니다."}</div>}</div></>}
