...
type Place={id:string;name:string;category:string;address?:string|null;latitude?:number|null;longitude?:number|null;distance_m?:number|null;walking_minutes?:number|null;editorial_note?:string|null;source_url?:string|null;verified?:boolean;curated?:boolean;review_signal?:number|null;review_samples?:ReviewSample[]};
type NearbyData={configured:boolean;curated:Place[];live:Place[];radius_m?:number;ranking?:string;review_signal_note?:string};
...
export default function NearbyPlaces({courseId,courseName,userId,initialFavoriteIds=[]}:{courseId:string;courseName:string;userId?:string|null;initialFavoriteIds?:string[]}){
 ...
 const places=useMemo(()=>{if(!data)return[];const seen=new Set<string>();return [...data.curated,...data.live].filter(p=>p.category===tab).filter(p=>{const k=`${p.name}|${p.address||""}`;if(seen.has(k))return false;seen.add(k);return true}).slice(0,12)},[data,tab]);
 ...
 return <section className="section nearbySection" id="after-run">
  <div className="sectionHead"><div><span className="eyebrow">AFTER RUN</span><h2>뛰고 나서 어디 갈까?</h2><p className="muted">코스 중심 최대 5km의 맛집·카페를 찾고, 실제 블로그 후기 신호가 많은 장소를 우선 추천합니다.</p></div><div className="segmented"><button className={tab==="restaurant"?"on":""} onClick={()=>setTab("restaurant")}>🍚 맛집</button><button className={tab==="cafe"?"on":""} onClick={()=>setTab("cafe")}>☕ 카페</button></div></div>
  {!data?<div className="card muted">주변 장소를 찾는 중...</div>:<><div className="nearbyTrustNote"><b>추천 기준</b><span>RUNART PICK 우선 · 블로그 후기 검색량 · 거리</span><small>{data.review_signal_note||"후기 검색량은 공식 평점이 아닌 참고 신호입니다."}</small></div><div className="placeRail">{places.map(p=><article className="placeCard" key={p.id}><div className="placeBadge">{p.curated?"RUNART PICK":p.review_signal&&p.review_signal>0?"USER SIGNAL":p.verified?"검증":"주변"}</div><h3>{p.name}</h3><p className="muted">{p.address||"주소 정보 없음"}</p><div className="metaRow"><span>{p.distance_m!=null?`${p.distance_m<1000?p.distance_m+"m":(p.distance_m/1000).toFixed(1)+"km"}`:"거리 미정"}</span>{p.walking_minutes&&<span>도보 {p.walking_minutes}분</span>}{p.review_signal!=null&&p.review_signal>0&&<span>후기 신호 {p.review_signal.toLocaleString()}+</span>}</div>{p.editorial_note&&<p className="placeNote">{p.editorial_note}</p>}{p.review_samples&&p.review_samples.length>0&&<div className="reviewSignalBox"><b>실사용 후기 참고</b>{p.review_samples.slice(0,2).map((r,i)=><a key={`${r.url}-${i}`} href={r.url} target="_blank" rel="noreferrer"><span>{r.title}</span><small>{r.blogname||"블로그 후기"}</small></a>)}</div>}<div className="actions"><button disabled={!!busy} className={`heartBtn ${favorites.has(p.id)?"saved":""}`} onClick={()=>toggle(p)}>{busy===p.id?"저장 중…":favorites.has(p.id)?"♥ 찜함":"♡ 찜"}</button><button disabled={!!busy} className="btn" onClick={()=>savePlan(p)}>{busy===`plan:${p.id}`?"일정 저장 중…":"RUN + EAT 저장"}</button>{p.source_url&&<a className="btn ghost" href={p.source_url} target="_blank" rel="noreferrer">지도 상세</a>}</div></article>)}{!places.length&&<div className="card muted">아직 연결된 {tab==="restaurant"?"맛집":"카페"}이 없습니다.{!data.configured&&" 장소 API 키를 연결하면 코스 주변을 자동 검색할 수 있습니다."}</div>}</div></>}
