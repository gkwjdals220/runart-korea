...
type LegacyParticipation={race_id:string;member_name:string;status:string;distance:string|null;applied:boolean;note:string|null;linked_user_id:string|null};
type Member={user_id:string;display_name:string;role:string};

export default function RaceManager({userId,crewId,role,races,participations,legacyParticipations,members}:{userId:string;crewId:string;role:string;races:Race[];participations:Participation[];legacyParticipations:LegacyParticipation[];members:Member[]}){
 ...
 const statusLabel=(s:string)=>s==="open"?"접수중":s==="closed"?"마감":s==="upcoming"?"접수예정":"미정";
 return <div className="stack">
  ...
  {races.map(r=>{const ps=participations.filter(p=>p.race_id===r.id);const legacy=legacyParticipations.filter(p=>p.race_id===r.id);const me=mine.get(r.id);const going=ps.filter(p=>p.status==="going");const legacyNames=legacy.filter(x=>!x.linked_user_id).map(x=>x.member_name);return <article className="card" key={r.id}><div className="courseTop"><div><h3>{r.name}</h3><p className="muted">{r.race_date||"날짜 미정"} · {r.region||"지역 미정"} {r.venue||""}</p></div><b>👥 {going.length}</b></div><div className="metaRow"><span>접수: {statusLabel(r.registration_status)}</span><span>마감: {r.registration_deadline||"미정"}</span>{r.distance_options?.map(d=><span className="tag" key={d}>{d}</span>)}</div><div className="actions"><button className={`btn ${me?.status==="going"?"":"ghost"}`} onClick={()=>setParticipation(r.id,"going")}>참가</button><button className={`btn ${me?.status==="maybe"?"":"ghost"}`} onClick={()=>setParticipation(r.id,"maybe")}>미정</button><button className={`btn ${me?.status==="not_going"?"":"ghost"}`} onClick={()=>setParticipation(r.id,"not_going")}>불참</button>{r.official_url&&<a className="btn ghost" href={r.official_url} target="_blank" rel="noreferrer">공식 페이지</a>}{admin&&<button className="btn danger" onClick={()=>deleteRace(r.id)}>삭제</button>}</div>{me?.status==="going"&&r.distance_options?.length>0&&<label>내 참가 종목<select value={me.distance||""} onChange={e=>setParticipation(r.id,"going",e.target.value)}><option value="">선택</option>{r.distance_options.map(d=><option key={d}>{d}</option>)}</select></label>}<p className="muted">현재 참가: {going.map(p=>members.find(m=>m.user_id===p.user_id)?.display_name||"러너").join(", ")||"아직 없음"}</p>{legacyNames.length>0&&<p className="muted">기존 보드 기록: {legacyNames.join(", ")} <span className="tag">미정</span></p>}{r.memo&&<p>{r.memo}</p>}</article>})}
