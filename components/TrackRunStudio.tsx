...
export default function TrackRunStudio({userId}:{userId?:string|null}){
 const [tracks,setTracks]=useState<Track[]>([]),[trackId,setTrackId]=useState(""),[presetId,setPresetId]=useState("free"),[rest,setRest]=useState(0),[resting,setResting]=useState(false),[loading,setLoading]=useState(true),[msg,setMsg]=useState(""),[runState,setRunState]=useState({running:false,finished:false});
 ...
 const handleRunStateChange=useCallback((running:boolean,finished:boolean)=>setRunState(current=>current.running===running&&current.finished===finished?current:{running,finished}),[]);
 const runActive=runState.running||runState.finished;
 ...
 return <div className="trackStudioPage">
  ...
  <RunModeV2 trackRun courseName={runName} userId={userId||null} onRunStateChange={handleRunStateChange} trackTools={preset.restSec>0?<section className="card trackRestTimer"><span className="eyebrow">REST TIMER</span><div className="trackRestTimerBody"><div><h2>{resting?fmt(rest):fmt(preset.restSec)}</h2><p className="muted">{resting?"회복 중 · 00:00이 되면 다음 반복을 시작하세요.":"한 반복을 마치면 휴식 시작을 눌러주세요."}</p></div><div className="actions"><button className="btn" type="button" onClick={startRest}>{resting?"↻ 휴식 다시 시작":"⏱ 휴식 시작"}</button>{resting&&<button className="btn ghost" type="button" onClick={resetRest}>휴식 종료</button>}</div></div></section>:null}/>
 </div>;
}
