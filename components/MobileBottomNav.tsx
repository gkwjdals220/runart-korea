...
const items=[
 {href:"/",label:"홈",icon:"home" as IconName,match:(p:string)=>p==="/"},
 ...
 {href:"/dashboard",label:"크루",icon:"crew" as IconName,match:(p:string)=>p.startsWith("/dashboard")||p.startsWith("/manage")||p.startsWith("/races")},
...
export default function MobileBottomNav(){
 ...
 return <nav className="mobileBottomNav mobileBottomNavV3" aria-label="모바일 주요 메뉴">
  {items.map(item=>{
   ...
   return <Link key={item.label} href={item.href} className={on?"active":""} aria-label={item.label} aria-current={on?"page":undefined}>
    <span className="mobileNavIcon"><NavIcon name={item.icon} active={on}/></span>
    <b>{item.label}</b>
    <i className="mobileNavActiveDot" aria-hidden="true"/>
   </Link>;
  })}
 </nav>;
}
