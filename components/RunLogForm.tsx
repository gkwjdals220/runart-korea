...
export default function RunLogForm({userId,crewId,courses,members,role}:{userId:string,crewId:string,courses:Course[],members:Member[],role?:string}){
 ...
 const [participants,setParticipants]=useState<string[]>([userId]); const [saving,setSaving]=useState(false);
 ...
 async function save(){
  ...
  if(participantInsert.error){setSaving(false);return setMsg(`러닝은 저장됐지만 참여자 저장 실패: ${participantInsert.error.message}`)}
  if(photo){
  ...
  setMemo("");setDistance("");setPhoto(null);setParticipants([userId]);setMsg("수행 기록과 참여자 출석이 저장되었습니다.");setSaving(false);router.refresh();
 }
 return <div className="card"><h3>러닝 수행 기록</h3><p className="muted">코스를 달린 뒤 실제 참여한 크루원을 체크해 출석을 남기세요.</p>
  ...
  <div className="attendanceBox"><div style={{display:"flex",justifyContent:"space-between",gap:10,alignItems:"center",marginBottom:10}}><b>참여 크루원</b><span className="muted">{participants.length}명 선택 · {role||"member"}</span></div><div className="attendanceGrid">{members.map(m=><label className={`memberCheck ${participants.includes(m.user_id)?"checked":""}`} key={m.user_id}><input type="checkbox" checked={participants.includes(m.user_id)} onChange={()=>toggleParticipant(m.user_id)}/><span><b>{m.display_name}</b><small>{m.role}</small></span></label>)}</div></div>
  ...
  <button className="btn" disabled={saving} onClick={save}>{saving?"저장 중...":"수행 + 참여자 저장"}</button>{msg&&<p className="muted">{msg}</p>}
 </div>
}