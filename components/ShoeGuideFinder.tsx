...
export default function ShoeGuideFinder(){
 ...
 return <section className="shoeFinder">
  ...
  <div className="shoeQuestion"><b><i>1</i> 주로 어떤 러닝을 하나요?</b><div className="shoeChoices" aria-label="러닝 목적">{types.map(x=><button type="button" aria-pressed={t===x.id} className={t===x.id?"on":""} onClick={()=>setT(x.id)} key={x.id}><strong>{x.label}</strong><span>{x.help}</span></button>)}</div></div>
  <div className="shoeQuestion"><b><i>2</i> 발 모양과 착화감은 어떤가요?</b><div className="shoeChoices foot" aria-label="족형">{feet.map(x=><button type="button" aria-pressed={f===x.id} className={f===x.id?"on":""} onClick={()=>setF(x.id)} key={x.id}><strong>{x.label}</strong><span>{x.help}</span></button>)}</div></div>
