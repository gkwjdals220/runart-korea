...
type Race={source_key:string;name:string;race_date:string;region:string|null;distance_options:string[];registration_deadline:string|null;registration_status:string;official_url:string|null;registration_url?:string|null;registration_text?:string|null;source_name:string;source_url:string};
...
const DISTANCE_FILTERS=["전체","Full","Half","10K","5K","기타"] as const;type DistanceFilter=(typeof DISTANCE_FILTERS)[number];
function normalizeDistance(v:string|null|undefined){return v?.trim().toLowerCase()==="ultra"?"Full":v||""}
...
function distanceMatches(r:Race,filter:DistanceFilter):boolean{if(filter==="전체")return true;const values=(r.distance_options||[]).map(d=>normalizeDistance(d).toLowerCase()),joined=values.join(" ");if(filter==="Full")return /(^|\s)(full|42k|42\.195k|42\.195)(\s|$)/i.test(joined);if(filter==="Half")return /(^|\s)(half|21k|21\.0975k)(\s|$)/i.test(joined);if(filter==="10K")return values.some(v=>/^10k$/i.test(v));if(filter==="5K")return values.some(v=>/^5k$/i.test(v));return !distanceMatches(r,"Full")&&!distanceMatches(r,"Half")&&!distanceMatches(r,"10K")&&!distanceMatches(r,"5K")}
...
export default function PublicRaceBrowser({userId,initialSaved,initialMineOnly=false}:{userId:string|null;initialSaved:Saved[];initialMineOnly?:boolean}){
 ...
 return <div className="publicRaceBrowser raceListV2">
  ...
  {error&&<div className="card raceError"><b>{error}</b></div>}
  <div className="raceMonthCards">{filtered.map(r=>{const me=saved.get(r.source_key);return <article className={`raceListItem ${me?"savedRace":""}`} key={r.source_key}><div className="raceListDate"><b>{dayOfMonth(r.race_date)}일</b><span>{weekday(r.race_date)}</span></div><div className="raceListContent"><div className="raceListTitleRow"><div><p>{r.region||"지역 미정"}</p><h3>{r.name}</h3></div><span className={`raceRegStatus ${r.registration_status}`}>{REG[r.registration_status]||"확인"}</span></div><div className="raceListDistances">{(r.distance_options||[]).map(d=><span key={d}>{normalizeDistance(d)}</span>)}</div><div className="raceListMeta">{r.registration_deadline&&<span>마감 {r.registration_deadline}</span>}<span>{dday(r.race_date)}</span></div>{r.registration_text&&<p className="raceListDescription">{r.registration_text}</p>}{userId&&<div className="raceParticipationLine"><button type="button" className={me?.status==="applied"?"active":""} onClick={()=>setParticipation(r,"applied")}><span className="miniCheck">{me?.status==="applied"?"✓":""}</span>신청완료</button><button type="button" className={me?.status==="going"?"active":""} onClick={()=>setParticipation(r,"going")}><span className="miniCheck">{me?.status==="going"?"✓":""}</span>참가예정</button></div>}<div className="racePrimaryLinks">{r.registration_url&&<a href={r.registration_url} target="_blank" rel="noreferrer">신청</a>}{r.official_url&&<a href={r.official_url} target="_blank" rel="noreferrer">대회 정보</a>}</div></div></article>})}</div>
  {!loading&&!error&&!filtered.length&&<div className="card emptyState"><b>{monthLabel(selectedMonth)} 일정이 없어요.</b><p className="muted">좌우 버튼으로 다른 달을 확인해보세요.</p></div>}
 </div>
}
