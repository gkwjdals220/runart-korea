...
export default function CourseExplorer({courses,userId,favoriteIds=[]}:{courses:Course[];userId?:string|null;favoriteIds?:string[]}){
 ...
 useEffect(()=>{const courseId=new URLSearchParams(window.location.search).get("course");if(courseId&&courses.some(c=>c.id===courseId)){setSelected(courseId);setMobileView("map");setTimeout(()=>document.getElementById("explore")?.scrollIntoView({block:"start"}),80)}},[courses]);
