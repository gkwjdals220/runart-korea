...
function nextSheetLevel(level:SheetLevel,direction:"up"|"down"):SheetLevel{
...
export default function CourseExplorer({courses,userId,favoriteIds=[]}:{courses:Course[];userId?:string|null;favoriteIds?:string[]}){
 ...
 const dragStartY=useRef<number|null>(null);const draggedRef=useRef(false);
 ...
 const [sheetLevel,setSheetLevel]=useState<SheetLevel>("half");const [sheetDragY,setSheetDragY]=useState(0);const [sheetDragging,setSheetDragging]=useState(false);
 ...
 const selectedRoutePoints=(selectedCourse?.route_geojson?.coordinates||[]).filter((x:any)=>Array.isArray(x)&&x.length>=2).length;
 ...
 useEffect(()=>{const courseId=new URLSearchParams(window.location.search).get("course");if(courseId&&courses.some(c=>c.id===courseId)){setSelected(courseId);setSheetLevel("half");setMobileView("map");setTimeout(()=>document.getElementById("explore")?.scrollIntoView({block:"start"}),80)}},[courses]);
 useEffect(()=>{if(selected)setSheetLevel("half")},[selected]);
 ...
 useEffect(()=>{let cancelled=false;(async()=>{const map=mapRef.current;if(!map||!mapReady)return;requestAnimationFrame(()=>map.invalidateSize(false));const L=await import("leaflet");if(cancelled)return;layersRef.current.forEach(x=>map.removeLayer(x));layersRef.current=[];const bounds:any[]=[];
   filtered.forEach(c=>{
    ...
    if(coords.length){const line=L.polyline(coords,{weight:selected===c.id?7:4,opacity:selected===c.id?1:.72}).addTo(map);line.bindTooltip(`${c.name} · ${Number(c.distance_km).toFixed(1)}km`,{sticky:true});line.on("click",()=>{setSelected(c.id);setMobileView("map")});layersRef.current.push(line);coords.forEach((x:any)=>bounds.push(x));const first=coords[0];const start=L.circleMarker(first,{radius:selected===c.id?8:6,weight:3,fillOpacity:1}).addTo(map).bindTooltip(`📍 ${c.start_name||c.name}`);start.on("click",()=>{setSelected(c.id);setMobileView("map")});layersRef.current.push(start)}
    else{const loc=courseLocations[c.id];if(!loc)return;const point:[number,number]=[loc.lat,loc.lng];const marker=L.circleMarker(point,{radius:selected===c.id?10:7,weight:3,fillOpacity:.92}).addTo(map).bindTooltip(`📍 ${c.name}<br/>${c.start_name||loc.label||"출발점"}`);marker.on("click",()=>{setSelected(c.id);setMobileView("map")});layersRef.current.push(marker);bounds.push(point)}
   });
   if(selected){const c=courses.find(x=>x.id===selected);const route=(c?.route_geojson?.coordinates||[]).filter((x:any)=>Array.isArray(x)&&x.length>=2);const mobile=typeof window!=="undefined"&&window.matchMedia("(max-width: 600px)").matches;const sheetPad=mobile?(sheetLevel==="full"?390:sheetLevel==="half"?245:130):35;if(route.length){const latlngs=route.map((x:number[])=>[x[1],x[0]]);map.fitBounds(latlngs,{paddingTopLeft:[30,30],paddingBottomRight:[30,sheetPad],maxZoom:15,animate:false})}else{const loc=courseLocations[selected];if(loc){map.setView([loc.lat,loc.lng],14,{animate:false});if(mobile)map.panBy([0,-Math.max(0,(sheetPad-100)/2)],{animate:false})}}}
  ...
  })();return()=>{cancelled=true}},[filtered,selected,mapReady,courseLocations,courses,sheetLevel]);
 ...
 function handleSheetPointerDown(e:React.PointerEvent<HTMLButtonElement>){dragStartY.current=e.clientY;draggedRef.current=false;setSheetDragging(true);e.currentTarget.setPointerCapture(e.pointerId)}
 function handleSheetPointerMove(e:React.PointerEvent<HTMLButtonElement>){if(dragStartY.current==null)return;const delta=Math.max(-120,Math.min(120,e.clientY-dragStartY.current));if(Math.abs(delta)>5)draggedRef.current=true;setSheetDragY(delta)}
 function handleSheetPointerUp(e:React.PointerEvent<HTMLButtonElement>){if(dragStartY.current==null)return;const delta=e.clientY-dragStartY.current;if(Math.abs(delta)>=44)setSheetLevel(v=>nextSheetLevel(v,delta<0?"up":"down"));else if(!draggedRef.current)setSheetLevel(v=>v==="full"?"peek":nextSheetLevel(v,"up"));dragStartY.current=null;setSheetDragY(0);setSheetDragging(false);try{e.currentTarget.releasePointerCapture(e.pointerId)}catch{}}
 ...
 return <div className="explorer explorerV2" id="explore">
  ...
  <div className={`mapLayout explorerLayout view-${mobileView}`}>
   <div className="mapPanel explorerMapPanel"><div className="mapFloatingActions"><button onClick={locateMe}>◎ 내 위치</button><button onClick={()=>setSelected(null)}>전국 보기</button></div><div ref={mapEl} className="courseMap"/>
    {selectedCourse&&<div className={`mapCourseInfo sheet-${sheetLevel} ${sheetDragging?"sheet-dragging":""}`} style={{"--sheet-drag-y":`${sheetDragY}px`} as React.CSSProperties}><button className="sheetHandle" aria-label="코스 정보 패널 크기 조절" onPointerDown={handleSheetPointerDown} onPointerMove={handleSheetPointerMove} onPointerUp={handleSheetPointerUp} onPointerCancel={handleSheetPointerUp}><span/><small>{sheetLevel==="peek"?"위로 밀어 보기":sheetLevel==="full"?"아래로 밀어 지도 보기":"위로 상세 · 아래로 축소"}</small></button><button className="mapCourseClose" aria-label="선택 해제" onClick={()=>setSelected(null)}>×</button><div className="mapCourseTitle"><div><small>{selectedCourse.region} {selectedCourse.city||""}</small><h3>{selectedCourse.name}</h3>{selectedCourse.start_name&&<p>📍 출발 {selectedCourse.start_name}</p>}{selectedLocation&&<p className="mapCoords">좌표 {selectedLocation.lat.toFixed(6)}, {selectedLocation.lng.toFixed(6)} {selectedLocation.source==="kakao"?"· 시작점 기준":"· GPS 경로 기준"}</p>}</div><strong>{Number(selectedCourse.distance_km).toFixed(1)}K</strong></div><div className="runnerFlowStrip"><span><b>📍 출발</b><small>{selectedCourse.start_name||"출발점"}</small></span><span><b>🧭 경로</b><small>{selectedRoutePoints?`${selectedRoutePoints}개 지점`:"시작점 기준"}</small></span><span><b>🚻 화장실</b><small>{toiletLoading?"조회 중":showToilets?`${toiletCount}곳`:"숨김"}</small></span><span><b>🚗 주차</b><small>{parkingIds.has(selectedCourse.id)?"900m 내 확인":"상세 확인"}</small></span><Link href={`/courses/${selectedCourse.id}#after-run`}><b>🍚 RUN+EAT</b><small>러닝 후 장소</small></Link></div><div className="mapCourseStats"><span>난이도 {"★".repeat(selectedCourse.difficulty||2)}</span><span>🚦 {selectedCourse.traffic_lights??"-"}</span><span>🚻 {showToilets?toiletCount:(selectedCourse.toilets??"-")}</span>{parkingIds.has(selectedCourse.id)&&<span>🚗 주차 편함</span>}{selectedCourse.elevation_gain_m!=null&&<span>↗ {selectedCourse.elevation_gain_m}m</span>}{selectedCourse.night_recommended&&<span>🌙 야간추천</span>}</div><div className="mapCourseTags">{(selectedCourse.tags||[]).slice(0,3).map(t=><span key={t}>#{t}</span>)}</div><div className="sheetExpandedDetail"><div><span>코스 형태</span><b>{selectedCourse.loop_type==="loop"?"순환":selectedCourse.loop_type==="out_back"?"왕복":"편도"}</b></div><div><span>노면</span><b>{selectedCourse.surface||"정보 없음"}</b></div><div><span>데이터</span><b>{selectedCourse.verified?"검증 완료":selectedCourse.data_quality||"확인 중"}</b></div></div><div className="mapCourseActions explorerMapActions"><Link href={`/courses/${selectedCourse.id}`}>상세 정보</Link><Link href={`/courses/${selectedCourse.id}#run-start`}>🏁 출발 준비</Link><Link href={`/courses/${selectedCourse.id}#after-run`}>🍚 RUN+EAT</Link><button onClick={()=>toggleFavorite(selectedCourse.id)}>{favorites.has(selectedCourse.id)?"♥":"♡"}</button></div></div>}
