...
export default function CourseExplorer({courses,userId,favoriteIds=[]}:{courses:Course[];userId?:string|null;favoriteIds?:string[]}){
 ...
 useEffect(()=>{let cancelled=false;(async()=>{const map=mapRef.current;if(!map||!mapReady)return;requestAnimationFrame(()=>map.invalidateSize(false));const L=await import("leaflet");if(cancelled)return;layersRef.current.forEach(x=>map.removeLayer(x));layersRef.current=[];const bounds:any[]=[];
   filtered.forEach(c=>{
    ...
    if(coords.length){
      const line=L.polyline(coords,{weight:selected===c.id?7:4,opacity:selected===c.id?1:.72}).addTo(map);line.bindTooltip(`${c.name} · ${Number(c.distance_km).toFixed(1)}km`,{sticky:true});line.on("click",()=>setSelected(c.id));layersRef.current.push(line);coords.forEach((x:any)=>bounds.push(x));
      const first=coords[0];const start=L.circleMarker(first,{radius:selected===c.id?8:6,weight:3,fillOpacity:1}).addTo(map).bindTooltip(`📍 ${c.start_name||c.name}`);start.on("click",()=>setSelected(c.id));layersRef.current.push(start);
    }else{
      const loc=courseLocations[c.id];if(!loc)return;const point:[number,number]=[loc.lat,loc.lng];
 ...
 async function toggleParkingFilter(){
  if(parkingFriendly){setParkingFriendly(false);return}
  if(parkingIds.size){setParkingFriendly(true);return}
  setParkingLoading(true);setMsg("출발지 근처 공영주차장을 확인하고 있어요.");
  try{const r=await fetch("/api/parking/coverage");const data=await r.json();const ids=new Set<string>(data.ids||[]);setParkingIds(ids);setParkingFriendly(true);setMsg(`출발점 900m 안에 공영·공공 주차장이 있는 ${ids.size}개 코스를 찾았어요.`)}catch{setMsg("주차 편한 코스 정보를 불러오지 못했습니다.")}finally{setParkingLoading(false)}
 }
 ...
 return <div className="explorer" id="explore"><div className="exploreTitle"><div><span className="eyebrow">EXPLORE</span><h2>내 취향대로 코스 찾기</h2></div><span className="muted">검색 · 내 주변 · 필터 · 지도</span></div><div className="filterbar">
  ...
  <button className={`chip ${parkingFriendly?"on":""}`} disabled={parkingLoading} onClick={toggleParkingFilter}>{parkingLoading?"🚗 주차 확인 중…":parkingFriendly?"🚗 주차 편한 코스 ON":"🚗 주차 편한 코스"}</button>
 ...
 </div>{locationsLoading&&<p className="muted">📍 코스 시작 위치를 불러오는 중…</p>}{msg&&<p className="muted">{msg}</p>}<div className="mapLayout"><div className="mapPanel"><div ref={mapEl} className="courseMap"/>{selectedCourse&&<div className="mapCourseInfo"><button className="mapCourseClose" aria-label="선택 해제" onClick={()=>setSelected(null)}>×</button><div className="mapCourseTitle"><div><small>{selectedCourse.region} {selectedCourse.city||""}</small><h3>{selectedCourse.name}</h3>{selectedCourse.start_name&&<p>📍 출발 {selectedCourse.start_name}</p>}{selectedLocation&&<p className="mapCoords">좌표 {selectedLocation.lat.toFixed(6)}, {selectedLocation.lng.toFixed(6)} {selectedLocation.source==="kakao"?"· 시작점 기준":"· GPS 경로 기준"}</p>}</div><strong>{Number(selectedCourse.distance_km).toFixed(1)}K</strong></div><div className="mapCourseStats"><span>난이도 {"★".repeat(selectedCourse.difficulty||2)}</span><span>🚦 {selectedCourse.traffic_lights??"-"}</span><span>🚻 {showToilets?toiletCount:(selectedCourse.toilets??"-")}</span>{parkingIds.has(selectedCourse.id)&&<span>🚗 주차 편함</span>}{selectedCourse.elevation_gain_m!=null&&<span>↗ {selectedCourse.elevation_gain_m}m</span>}{selectedCourse.night_recommended&&<span>🌙 야간추천</span>}</div><div className="mapCourseTags">{(selectedCourse.tags||[]).slice(0,3).map(t=><span key={t}>#{t}</span>)}</div><div className="mapCourseActions"><Link href={`/courses/${selectedCourse.id}`}>코스 상세 보기</Link><button onClick={()=>toggleFavorite(selectedCourse.id)}>{favorites.has(selectedCourse.id)?"♥ 찜됨":"♡ 찜하기"}</button></div></div>}<p className="mapDataNote">📍 GPS 경로가 없는 코스는 출발 장소 검색 좌표로 표시합니다. 🚻 화장실은 선택한 코스 주변의 OpenStreetMap 공공 데이터를 기준으로 표시됩니다.</p></div><div className="courseList"><div className="listHead"><b>{filtered.length}개 코스</b><span className="muted">{nearMode?"가까운 순":parkingFriendly?"주차 편한 코스":"RUNART Discovery"}</span></div>
 {filtered.map(c=>{const away=distanceMap.get(c.id);const hasRoute=(c.route_geojson?.coordinates||[]).length>0;const hasLocation=!!courseLocations[c.id];return <article key={c.id} className={`courseCard ${selected===c.id?"selected":""}`} onClick={()=>setSelected(c.id)}><div className="courseTop"><div><h3>{c.name} {c.verified&&<span className="done">✓</span>}</h3><p className="muted">{c.region} {c.city||""}{c.start_name?` · ${c.start_name}`:""}{nearMode&&away!=null?` · 내 위치 ${away.toFixed(1)}km`:""}</p></div><div className="courseCardActions"><b>{Number(c.distance_km).toFixed(1)}K</b><button aria-label="찜하기" className={`iconHeart ${favorites.has(c.id)?"saved":""}`} onClick={e=>{e.stopPropagation();toggleFavorite(c.id)}}>{favorites.has(c.id)?"♥":"♡"}</button></div></div><div className="metaRow"><span className="tag">{hasRoute?"🧭 GPS 경로":"📍 시작점"}{!hasRoute&&!hasLocation&&!locationsLoading?" 확인중":""}</span>{parkingIds.has(c.id)&&<span className="tag">🚗 주차 편함</span>}{c.course_type==="art"&&<span className="tag">🎨 {c.art_shape||"GPS ART"}</span>}<span>난이도 {"★".repeat(c.difficulty||2)}</span>{c.surface&&<span>노면 {c.surface}</span>}{c.elevation_gain_m!=null&&<span>↗ {c.elevation_gain_m}m</span>}<span>🚦 {c.traffic_lights??"-"}</span><span>🚻 {selected===c.id&&showToilets?toiletCount:(c.toilets??"-")}</span>{c.night_recommended&&<span>🌙</span>}</div><div className="metaRow">{(c.tags||[]).slice(0,4).map(t=><span className="tag" key={t}>#{t}</span>)}</div><Link href={`/courses/${c.id}`} className="textLink" onClick={e=>e.stopPropagation()}>코스 상세 · 주변 맛집 →</Link></article>})}{!filtered.length&&<div className="empty">조건에 맞는 코스가 없습니다.</div>}</div></div></div>
}
