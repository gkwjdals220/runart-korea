...
export default function CourseExplorer({courses}:{courses:Course[]}){
 ...
 const filtered=useMemo(()=>courses.filter(c=>{
  ...
  if(surface&&c.surface!==surface)return false;if(loop&&c.loop_type!==loop)return false;if(night&&!c.night_recommended)return false;if(verified&&!c.verified)return false;if(lowSignals&&(c.traffic_lights??99)>2)return false;return true;
 }),[courses,q,region,type,distance,surface,loop,night,verified,lowSignals]);
