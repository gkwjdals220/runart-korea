...
export default function ProfileEditor({userId,initialName,initialAvatarPath=null}:{userId:string;initialName:string;initialAvatarPath?:string|null}){
 ...
 const sbRef=useRef<ReturnType<typeof createClient>|null>(null);if(!sbRef.current)sbRef.current=createClient();const sb=sbRef.current;
 ...
 async function save(){
  ...
  try{await ensureSession();const {error}=await sb.from("runart_profiles").upsert({user_id:userId,display_name:v,avatar_path:avatarPath,updated_at:new Date().toISOString()});if(error)throw error;setName(v);setSavedName(v);setMsg("표시 이름을 저장했습니다.");}
