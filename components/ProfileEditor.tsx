...
export default function ProfileEditor({userId,initialName}:{userId:string;initialName:string}){
 ...
 return <div className="card profileEditorCard"><div><span className="eyebrow">DISPLAY NAME</span><h3>내 프로필</h3></div><label>표시 이름<input value={name} maxLength={40} disabled={saving} autoComplete="nickname" onChange={e=>{setName(e.target.value);setMsg("")}}/></label><small className="muted profileNameHint">러닝 기록과 크루 화면에 표시됩니다.</small><button className="btn" type="button" disabled={saving||unchanged||!name.trim()} onClick={save}>{saving?"저장 중…":unchanged?"현재 이름":"이름 저장"}</button>{msg&&<p className="muted formStatus" role="status" aria-live="polite">{msg}</p>}</div>
}
