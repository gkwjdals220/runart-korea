...
export default function ProfileEditor({userId,initialName,initialAvatarPath=null}:{userId:string;initialName:string;initialAvatarPath?:string|null}){
 ...
 const [avatarPath,setAvatarPath]=useState<string|null>(initialAvatarPath);const [avatarUrl,setAvatarUrl]=useState<string>("");const [uploading,setUploading]=useState(false);const fileRef=useRef<HTMLInputElement|null>(null);
 ...
 async function uploadAvatar(file?:File){
  ...
  try{
   const {error:uploadError}=await sb.storage.from("runart-media").upload(nextPath,file,{cacheControl:"3600",upsert:false,contentType:file.type});if(uploadError)throw uploadError;
  ...
  }catch(e:any){setMsg(e?.message||"프로필 사진 저장 중 오류가 발생했습니다.")}finally{setUploading(false);if(fileRef.current)fileRef.current.value=""}
 }
 ...
 return <div className="card profileEditorCard">
  ...
  <div className="profilePhotoRow">
   <button type="button" className="profilePhotoButton" onClick={()=>fileRef.current?.click()} disabled={uploading} aria-label="프로필 사진 변경">
    {avatarUrl?<img src={avatarUrl} alt="프로필 사진"/>:<span aria-hidden="true">+</span>}
   </button>
   <div className="profilePhotoMeta"><b>{uploading?"사진 업로드 중…":"프로필 사진"}</b><small className="muted">정사각형 이미지 권장 · 최대 5MB</small><button type="button" className="btn ghost profilePhotoAction" disabled={uploading} onClick={()=>fileRef.current?.click()}>{avatarUrl?"사진 변경":"사진 추가"}</button></div>
   <input ref={fileRef} className="profilePhotoInput" type="file" accept="image/*" onChange={e=>uploadAvatar(e.target.files?.[0])}/>
  </div>
