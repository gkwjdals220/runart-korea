...
export default function HomePromoCarousel({cards}:{cards:Card[]}){
 ...
 return <section className="homePromoCarousel" aria-label="주요 소식과 바로가기"><div ref={rail} className="homePromoRail" onScroll={sync} onPointerDown={()=>touching.current=true} onPointerUp={()=>touching.current=false} onPointerCancel={()=>touching.current=false}>{cards.map((c,i)=><Link className="homePromoCard homePromoImageCard" href={c.href} aria-label={c.label} key={`${c.href}-${i}`}><Image src={c.image} alt="" fill priority={i===0} sizes="(max-width: 700px) calc(100vw - 48px), 34vw" style={{objectFit:"contain",objectPosition:"center"}}/></Link>)}</div><div className="homePromoDots" aria-label="배너 위치">{cards.map((_,i)=><button type="button" key={i} aria-label={`${i+1}번째 배너`} className={active===i?"on":""} onClick={()=>go(i)}/>)}</div></section>
}
